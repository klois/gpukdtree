...
#define IO_NBLOCKS 30   /*!< total number of defined information blocks for snapshot files.
...
// 	IO_HEADER,
	...
	//now follow Arepo specific blocks
	IO_VOL,
	IO_GRADV,
	IO_PRESSURE,
	IO_DIVVEL,
	...
	IO_HIGHRESMASS,
	IO_RPS,
	IO_GRADR, //density gradient
	IO_GRADP,  //pressure gradient
	IO_FACEANGLE
};
...
typedef struct  
{
	 ...
	 float 	Accel[3];
	 int   	Type; 		//not in snapshot file
	 float 	r; 			//just for testing, not in snapshot file
	 ...
	 float Temp;	//temperature, not in snapshot file
	...
	//AREPO properties
	float Volume;	//AREPO: volume of grid cell
