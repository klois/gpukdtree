...
int unitConversion(particle_data *P, int size, int cooling) //cooling enabled? if not, assume fully ionized gas
{
	...
	//no cooling and fully ionized gas
	if(!cooling)
	{
		double yhelium = (1.0 - XH) / (4.0 * XH);
		MeanWeight = (1 + 4 * yhelium) / (2 + 3.0 * yhelium) * PROTONMASS;
	}
	
	//printf("temperature conversion factor: %g\n", MeanWeight/BOLTZMANN * (gamma-1) * UnitEnergy_in_cgs/ UnitMass_in_g);
...
int calcWakeDiskRatios(const particle_data *const P, const io_header *const header, const float wakeDist, InvestData *const idata, const float initNsMass, const float phi, const float h, const int first_run)
{
     ...
     if((id_p = id = (int*) realloc(id, sizeof(int)*size)) <= 0)
     {
...
inline float dist(const float *const a, const float *const b)
{
	float d[3] = {a[0] - b[0], a[1] - b[1], a[2] - b[2]};
	return sqrt(d[0]*d[0] + d[1]*d[1] + d[2]*d[2]);
}

void calcCentroid(particle_data *P, const int size, float *centroid)
{
     int j;
     double totMass = 0.0;
...
{
     return ((particle_data *)a)->r < ((particle_data *)b)->r ? -1 :  ((particle_data *)a)->r > ((particle_data *)b)->r ? 1 : 0;
}

/** start with initial guess of centroid and cluster radius provided to the function
 * 
 * \param steps max number of steps
 */
void shrinkingSpheres(float *centroid, particle_data *const P, const int size, const int steps, float initR)
{
     int j, lastSize = size;
     float newR = initR;
     
     centroid[0] = centroid[1] = centroid[2] = 0.0;
     calcCentroid(P, lastSize, centroid);
     ...
     for(j = 0; j < steps; ++j)
     {
	  ...
	  for(i = 0; i < lastSize; ++i)
	       P[i].r = dist(centroid, P[i].Pos);
	  qsort(P, lastSize, sizeof(particle_data), compare);
	  ...
	  calcCentroid(P, i, centroid);
	}
}

inline void crossP(const float *const a, const float *const b, float *const res)
{
...
inline float norm(float *const a)
{
...
inline float dotP(const float *const a, const float *const b)
{
	return a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
}

void calcL(const particle_data *const P, const int size, double L[3], const float *const centroid)
{
	int i, j;
	...
	//calc galaxy rest frame
	/*
	for(j = 0; j < size; ++j)
		//if(dist(P[j].Pos, centroid) < 20 && P[j].Rho*1.989e43/2.938e64 > 4e-26 && P[j].Temp < 1e6)
		{
			for(i = 0; i < 3; ++i)
				vel[i] += P[j].Vel[i]*P[j].Mass;
			totMass += (double)P[j].Mass;
		}
		
	if(totMass == 0.0)
	{
		fprintf(stderr, "mass equals zero!\n");
		exit(-1);
	}
	for(i = 0; i < 3; ++i)
		vel[i] /= totMass;
	
	printf("\tcalcL: relative velocity: %g %g %g\n", vel[0], vel[1], vel[2]);
	*/
	...
	for(j = 0; j < size; ++j)
	{
		//if(dist(P[j].Pos, centroid) < 20 && P[j].Rho*1.989e43/2.938e64 > 4e-26 && P[j].Temp < 1e6)
		{
			float r[3] = {P[j].Pos[0]-centroid[0], P[j].Pos[1] - centroid[1], P[j].Pos[2] - centroid[2]};
			float mv[3] = {(P[j].Vel[0]-vel[0])*P[j].Mass, (P[j].Vel[1] - vel[1])*P[j].Mass, (P[j].Vel[2] - vel[2])*P[j].Mass};
			float res[3];
	...
	printf("\tcalcL-- J: %g for mass: %g\n", sqrt(L[0]*L[0] + L[1]*L[1] + L[2]*L[2]), totMass);
}
