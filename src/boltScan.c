...
void scan(icl_buffer *data, icl_buffer *flag, UINT n) {
	...
	icl_run_kernel(perBlockScanByKey, 1, &gx, &wx, NULL, NULL, 6,
			(size_t)0, (void *)flag,
			(size_t)0, (void *)data,
			sizeof(INT), &init,
			sizeof(UINT), &n,
...
*/
	...
	workPerThread = sizeScanBuff / wx;

	icl_run_kernel(intraBlockInclusiveScanByKey, 1, &wx, &wx, NULL, NULL, 5,
			(size_t)0, (void *)keySumArray,
			(size_t)0, (void *)preSumArray,
...
*/
	...
	icl_run_kernel(perBlockAdditionByKey, 1, &gx, &wx, NULL, NULL, 5,
			(size_t)0, (void *)keySumArray,
			(size_t)0, (void *)postSumArray,
			(size_t)0, (void *)flag,
			(size_t)0, (void *)data,
...
//	perBlockScanTime += icl_profile_event(perBlockScanEvent, ICL_STARTED, ICL_FINISHED, ICL_MILLI);
//	intraBlockTime += icl_profile_event(intraBlockEvent, ICL_STARTED, ICL_FINISHED, ICL_MILLI);
//	perBlockAdditionTime += icl_profile_event(perBlockAdditionEvent, ICL_STARTED, ICL_FINISHED, ICL_MILLI);
#endif
}
...
*/
	wx = chunk_size;
	UINT numWorkGroups, sizeScanBuff;
	UINT buf_1;
	dev = _dev;

	// overapproximation for allocation, using maximum allowed size for n
	numWorkGroups = ((maxN + wx - 1) / wx);
	sizeScanBuff = ((numWorkGroups + wx -1) / wx) * wx;
	buf_1 = sizeof(UINT)*sizeScanBuff;
